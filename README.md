...
#### 1.1. На основе представленых виртуальных машин или шаблонов виртуальных машин создайте отсутствующие виртуальные машины в соответствии со схемой.
   - a. Характеристики виртуальных машин установите в соответствии с [Таблицей 1](https://github.com/Dauxdu/DEMO2023#%D1%82%D0%B0%D0%B1%D0%BB%D0%B8%D1%86%D0%B0-1-%D1%85%D0%B0%D1%80%D0%B0%D0%BA%D1%82%D0%B5%D1%80%D0%B8%D1%81%D1%82%D0%B8%D0%BA%D0%B8-vms);
...
#### 2.1. Настройсте статический маршрут по умолчению на маршрутизаторах ***RTR-L*** и ***RTR-R***.
...
#### 1.1. Сети, подключённые к ***ISP***, считаются внешними:
...
#### 1.2. Обеспечьте настройку служб SSH региона Left:
   - а. Подключение со стороны внешних сетей по протоколу к платформе управления трафиком ***RTR-L*** на порт `2222` должны быть перенаправлены на ***WEB-L***;
   - b. Подключение со стороны внешних сетей по протоколу к платформе управления трафиком ***RTR-R*** на порт `2244` должны быть перенаправлены на ***WEB-R***.
...
#### 2.1. Выполните настройку первого уровня DNS-системы стенда:
   ...
   - c. Сервер делигирует зону `int.demo.wsr` на ***SRV***;
   ...
   - d. Внешний клиент ***CLI*** должен использовать DNS-службы, развёрнутую на ***ISP***, по умолчанию.
...
#### 2.3. Выполните настройку первого уровня системы сихронизации времени:
   - а. Используйте ***ISP***;
...
#### 2.4. Выполните настройку второго уровня системы сихронизации времени на ***SRV***:
   - а. Сервер синхронизирует время с ***ISP***;
   ...
   - c. Все внутренние хосты (в том числе и платформы управления трафиком) должны синхронизировать своё время с ***SRV***.
#### 2.5. Реализуйте файловый SMB-сервер на базе ***SRV***
   - а. Сервер должен предоставлять доступ для обмена файлами серверам ***WEB-L*** и ***WEB-R***;
...
#### 2.6. Сервера ***WEB-L*** и ***WEB-R*** должны использовать службу, настроенную на ***SRV***, для обмена файлов между собой
...
### 3. Взаимодействие со специалистами смежного профиля при разработке методов, средсвт и технологий применения объектов профессиональной деятельности
...
#### 3.4. Необходимо реализовать следующую инфраструктуру приложения:
   ...
   - b. Хостинг приложения осуществляется на виртуальной машинах ***WEB-L*** и ***WEB-R***;
...
#### 3.5. Необходимо обеспечить отказоустойчивость приложения:
   - a. Сайт должен продолжать обслуживание (с задержкой не более 25 секунд) в следующих сценариях.
     - Отказ одной из виртуальной машины ***WEB***;
...
#### 1.1. Защита должна обеспечиваться с помощью IPsec между платформами ***RTR-L*** И ***RTR-R***
